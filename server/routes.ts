...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 戰情室 Dashboard API（公開，維持原本無需驗證的行為）
    ...
    try {
      ...
      res.json({
        ...
        users: users.map(u => ({
          ...
          canUseAiAgent: u.canUseAiAgent === 'true',
  ...
  app.post('/api/admin/interview-users', async (req, res) => {
    try {
      const { db } = await import('./db');
      const { interviewAuthorizedUsers } = await import('@shared/schema');
      const { userId, userName, canInterviewCheck = true, canInternalQuery = false, canUseAiAgent = false } = req.body ?? {};
      if (!userId || !userName) {
      ...
      const [created] = await db.insert(interviewAuthorizedUsers).values({
        ...
        canUseAiAgent: canUseAiAgent === true ? 'true' : 'false',
      }).returning();
      res.json({ success: true, user: { ...created, isActive: created.isActive === 'true', canInterviewCheck: created.canInterviewCheck === 'true', canInternalQuery: created.canInternalQuery === 'true', canUseAiAgent: created.canUseAiAgent === 'true' } });
    } catch (error: any) {
  ...
  app.patch('/api/admin/interview-users/:userId', async (req, res) => {
    try {
      ...
      const { isActive, canInterviewCheck, canInternalQuery, canUseAiAgent, userName } = req.body ?? {};
      ...
      if (typeof canUseAiAgent === 'boolean') updateData.canUseAiAgent = canUseAiAgent ? 'true' : 'false';
  ...
  // 面試授權用戶 — 停用（保留相容 DELETE 路徑，但不做實體刪除）
  app.delete('/api/admin/interview-users/:userId', async (req, res) => {
    try {
      const { db } = await import('./db');
      const { interviewAuthorizedUsers } = await import('@shared/schema');
      const { eq } = await import('drizzle-orm');
      const [disabled] = await db.update(interviewAuthorizedUsers)
        .set({ isActive: 'false', updatedAt: new Date() })
        .where(eq(interviewAuthorizedUsers.userId, req.params.userId))
        .returning();
      if (!disabled) return res.status(404).json({ success: false, error: '找不到該用戶' });
      res.json({ success: true, action: 'disabled', userId: req.params.userId });
    } catch (error) {
      res.status(500).json({ success: false, error: (error as Error).message });
    }
  });
...
function buildClockInFlexCard(): object {
  const logoUrl = `${getAppBaseUrl()}/images/juns-logo.jpg`;
  return {
    type: 'flex',
    altText: '🏃 駿斯運動 — 上下班打卡',
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          {
            type: 'text',
            text: '如有異常 ⚠️，請先「截圖」打卡畫面後，回傳400帳號。',
            wrap: true,
            size: 'sm',
            color: '#5a7a9a',
            margin: 'sm',
