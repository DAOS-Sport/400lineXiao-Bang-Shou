...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========== LINE 內部瀏覽器守門 ==========
  // 前端頁面只允許 LINE 內部瀏覽器存取；API / webhook / 靜態資產不受限制
  app.use((req, res, next) => {
    const path = req.path;

    // 放行：後端 API、Webhook、健康檢查、靜態資產、Vite HMR
    if (
      path.startsWith('/api/') ||
      path.startsWith('/webhook') ||
      path.startsWith('/health') ||
      path.startsWith('/attached_assets/') ||
      path.startsWith('/@') ||          // Vite 開發模組
      path.startsWith('/src/') ||
    ...
    // 檢查 User-Agent 是否為 LINE 內部瀏覽器
    const ua = req.headers['user-agent'] || '';
    const isLineApp = /Line\//i.test(ua) || /LIFF/i.test(ua);

    if (!isLineApp) {
      console.log(`🚫 非 LINE 瀏覽器擋下: ${req.method} ${path} | UA: ${ua.slice(0, 80)}`);
      res.status(403).send(`<!DOCTYPE html>
<html lang="zh-TW">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>請使用 LINE 開啟</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'PingFang TC', 'Helvetica Neue', sans-serif;
      background: linear-gradient(135deg, #06c755 0%, #1a3a5c 100%);
      min-height: 100vh;
      display: flex;
      align-items: center;
...
<body>
  <div class="card">
    <div class="icon">🔒</div>
    <h1>僅限 LINE 內部開啟</h1>
    <p>此頁面僅供駿斯人員<br>透過 LINE 訊息連結存取。</p>
    <p>請在 LINE 對話中<br>點擊連結或按鈕進入。</p>
    <div class="badge">請從 LINE App 開啟</div>
  </div>
</body>
</html>`);
      return;
  ...
  // 1. 全域功能模組狀態
  app.get("/api/admin/dashboard/global-apps", async (req, res) => {
    try {
  ...
  // 2. 私人專屬與權限服務狀態
  app.get("/api/admin/dashboard/private-services", async (req, res) => {
    try {
      const today = new Date(); today.setHours(0, 0, 0, 0);
  ...
  // 3. 實體場館自動化矩陣
  app.get("/api/admin/dashboard/venue-automations", async (req, res) => {
    try {
      const venues = [
        {
          venue: "竹科戶外游泳池",
  ...
  // 4. 微服務健康度檢查
  app.get("/api/admin/dashboard/services-health", async (req, res) => {
    try {
